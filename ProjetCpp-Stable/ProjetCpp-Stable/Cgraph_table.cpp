...
void Cgraph_table::GRTempty_table(){
	...
	while (uiIndex_of_graph < this->GRTget_nb_graph()){
		delete(this->GRTget_graph(uiIndex_of_graph));
		uiIndex_of_graph++;
	}
	delete(this->ppGRTgraph_table);
...
void Cgraph_table::GRTadd_graph(Cgraph * pGraph){
	...
	//realloc memory to add the graph to the table
	ppNew_graph_table = (Cgraph**)realloc(ppGRTgraph_table, sizeof(Cgraph*)*(uiGRTnb_graph + 1));
	if (ppNew_graph_table == NULL){
		Cexception exc(ERROR_REALLOCATION_MEMORY);
		throw exc;
	}
	else{
		ppGRTgraph_table = ppNew_graph_table;
		ppGRTgraph_table[uiGRTnb_graph] = pGraph;
		uiGRTnb_graph++;
	}
